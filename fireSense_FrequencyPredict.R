...
fireSense_FrequencyPredictInit <- function(sim) {
  ...
  sim <- scheduleEvent(sim, eventTime = p(sim)$initialRunTime, "fireSense_FrequencyPredict", "run")
  sim
...
fireSense_FrequencyPredictRun <- function(sim) {
  ...
  if (all(unlist(lapply(allxy, function(x) is.vector(envData[[x]]))))) {
    ...
    if (any(!exist)) {
      stop(paste0("fireSense_FrequencyPredict> Variable '", allxy[which(!exist)[1L]], "' not found."))
    } else if (any(class)) {
      stop("fireSense_FrequencyPredict> Data objects are not of the same class (e.g. data.frames).")
    } else {
      stop(paste0("fireSense_FrequencyPredict> Variable '", allxy[which(!class)[1L]], "' does not match a data.frame's column, a list component, or a RasterLayer."))
    }
  }
