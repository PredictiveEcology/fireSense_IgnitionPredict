...
# Overview
Predicts fire frequency or rates of fire counts using a model fitted with the fireSense_FrequencyFit module. Use them to feed the ignition component of a landscape fire model (e.g fireSense).
...
----|:-------|---------------------------------------------------------------------
`modelObjName`|`"fireSense_FrequencyFitted"`|name of the object of class fireSense_FrequencyFit describing the statistical model used for predictions.
`data`|`"dataFireSense_FrequencyPredict"`|a character vector indicating the names of objects in the `simList` environment in which to look for variables present in the model formula. `data` objects can be data.frames, RasterStacks or RasterLayers. However, data.frames cannot be mixed with objects of other classes.
...
- **dataFireSense_FrequencyPredict**: one or more data.frames, RasterLayers, RasterStacks or RasterBricks in which to look for variables with which to predict.
